...
const CATEGORY_STYLES: Record<string, { bg: string; icon: string; iconBg: string }> = {
  cardloan: { bg: 'from-red-600 to-rose-700', icon: '💳', iconBg: 'bg-white/20' },
  bank: { bg: 'from-blue-600 to-indigo-700', icon: '🏦', iconBg: 'bg-white/20' },
  sarakin: { bg: 'from-orange-500 to-red-600', icon: '💰', iconBg: 'bg-white/20' },
  refinance: { bg: 'from-purple-600 to-violet-700', icon: '🔄', iconBg: 'bg-white/20' },
}

export default function ArticleCard({ article }: { article: Article }) {
  const style = CATEGORY_STYLES[article.category] || CATEGORY_STYLES.cardloan
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        <h3 className="font-bold text-gray-900 text-sm leading-relaxed mb-3 line-clamp-3 group-hover:text-red-700 transition-colors">
          {article.title}
        </h3>
