...
const SITE = {
  name: 'カードローン比較ナビ',
  url: 'https://loan-select.com',
};

const AFFILIATE_TOP = `
<div style="background:#fff1f2;border:2px solid #dc2626;border-radius:8px;padding:16px;margin:24px 0;">
  <p style="font-weight:bold;color:#7f1d1d;margin:0 0 8px;">【PR】おすすめカードローンを今すぐ比較</p>
  <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:8px;">
    <li style="color:#374151;font-size:14px;">※ 現在アフィリエイトリンクを準備中です。最新の比較情報は各カードローン会社の公式サイトをご確認ください。</li>
  </ul>
</div>`;

const AFFILIATE_BOTTOM = `
<div style="background:#fffbeb;border:2px solid #d97706;border-radius:8px;padding:16px;margin:24px 0;">
  <p style="font-weight:bold;color:#92400e;margin:0 0 12px;">📚 お金の管理・借り入れの参考書籍</p>
  <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:8px;">
...
async function generateArticle() {
  ...
  const message = await client.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `あなたはカードローン比較メディア「${SITE.name}」の専門ライターです。
